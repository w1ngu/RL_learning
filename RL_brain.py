...
class DQN:
    ...
    def _build_net(self):        #构建网络
        ...
        self.model2 = Model(eval_inputs, self.q_eval)   #只训练eval_net  model2
    ...
    def learn(self):
        ...
        # print('batch',batch_memory[:, -self.n_features:])
        q_next, q_eval = self.model1.predict(batch_memory[:, -self.n_features:]), self.model2.predict(batch_memory[:, :self.n_features])
        #  m1 预测下一个观测值 m2 预测现在的观测值  ？
        ...
        print(self.epsilon, self.epsilon_max, self.epsilon_increment)
    ...
    def plot_show(self):
        ...
        # plt.plot(x, y_b_1, 'ro-', x, y_b_2, 'g.-', x, y_b_3, 'b*-', x, y_b_4, 'y>-', x, meteor, 'm<-', x, rouge_l,
        #          'kx-', x, cider, 'c^-', x, spice, 'k+-)
    ...
    def save(self,name='m'):
        self.model1.save('model/' + name + '1.hdf5')
        self.model2.save('model/' + name + '2.hdf5')
